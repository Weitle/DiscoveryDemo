...
def incomes():
    # 根据账期获取各专业收入
    acct_month = "201905"
    subject_incomes = db.session.query(Income.subject, IncomeType.name, IncomeType.level, Income.income, Income.tax).filter(and_(Income.subject==IncomeType.id, IncomeType.level<=1)).all()
